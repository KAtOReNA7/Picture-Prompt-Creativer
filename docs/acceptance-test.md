...
- 前置条件：Prompt 库至少有 25 条记录。
- 操作步骤：打开 `/library`。
- 预期结果：默认每页显示 24 条或不足 24 条；底部显示当前范围、总数、页码和上一页/下一页。
- 失败排查：检查 `/library` 服务端分页查询和 `GET /api/analyses` 分页参数。

- 操作步骤：切换 `pageSize=48` 和 `pageSize=96`。
- 预期结果：URL 保留 `pageSize`，页面回到第 1 页并显示对应数量。
- 失败排查：检查筛选表单是否未提交旧 `page` 参数。

- 操作步骤：搜索关键词或选择标签筛选。
- 预期结果：页面回到第 1 页；刷新页面后搜索、标签和分页状态仍保留。
- 失败排查：检查 URL query 拼接和 `makeQuery`。

- 操作步骤：查看任意卡片。
- 预期结果：卡片只显示预览图、标题、最多 4 个标签、模块数量、迁移次数、版本数量、生成图数量、时间、查看详情和删除；不显示长篇风格摘要、主体描述、构图、reversePrompt。
- 失败排查：检查 `LibraryBulkManager` 卡片字段。

- 操作步骤：点击“查看详情”。
- 预期结果：进入 `/library/[id]`，详情页完整信息仍在。
- 失败排查：检查详情页是否仍读取完整 PromptAnalysis。

- 操作步骤：在当前页选择多条记录，执行批量删除、批量添加标签、批量加入合集、批量导出。
- 预期结果：所有批量操作仍可用，且只作用于当前页选择。
- 失败排查：检查 `selectedIds` 和批量 API 请求体。

- 操作步骤：选择若干记录后翻页或切换筛选。
- 预期结果：当前选择被清空，不会跨页误删。
- 失败排查：检查列表组件 key 是否随分页筛选变化重建。
